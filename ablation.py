...
def train_loop(dataloader, scheduler_bool=False):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if LEVEL == 1:
            loss = loss_function(model_output, batch['labels_level_1'])
        elif LEVEL == 2:
            loss = loss_function(model_output, batch['labels_level_2'])
        else:
            loss = loss_function(model_output, batch['labels_level_3'])
        ...
        if not batch_idx % 10:
            ...
            if LEVEL == 1:
                js = get_js_distance('Level-1', batch['labels_level_1'], model_output)
            elif LEVEL == 2:
                js = get_js_distance('Level-2', batch['labels_level_2'], model_output)
            else:
...
for i in range(3):
    ...
    if LEVEL == 1:
        model = Multi_IDDR_Classifier_Ablation(MODEL_NAME, NUMBER_OF_SENSES['level_1'])
    elif LEVEL == 2:
        model = Multi_IDDR_Classifier_Ablation(MODEL_NAME, NUMBER_OF_SENSES['level_2'])
    else:
        model = Multi_IDDR_Classifier_Ablation(MODEL_NAME, NUMBER_OF_SENSES['level_3'])
    ...
    if SCHEDULER == 'none':
        model_path = 'Models/DiscoGeM-2.0_'+LANG+'/ABLATION-'+str(LEVEL)+'_'+after_slash_model_name+'_'+str(LEARNING_RATE)+'_'+str(i+1)+'.pth'
    else:
        model_path = 'Models/DiscoGeM-2.0_'+LANG+'/ABLATION-'+str(LEVEL)+'_'+after_slash_model_name+'_'+str(LEARNING_RATE)+'_'+SCHEDULER+'_'+str(i+1)+'.pth'
